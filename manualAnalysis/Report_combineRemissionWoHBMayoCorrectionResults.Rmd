...
sigs.rxnExpr <- fread("./Statistics/results/rxnExpr_GLMMsRemissionWoHBMayoCorrection_coefs.csv")
sigs.rxnExpr <- sigs.rxnExpr[padj < 0.05,]
sigs.PA <- fread("./Statistics/results/PA_GLMMsRemissionWoHBMayoCorrection_coefs.csv")
sigs.PA <- sigs.PA[padj < 0.05,]
sigs.FVA <- fread("./Statistics/results/FVA_GLMMsRemissionWoHBMayoCorrection_coefs.csv")
...
hgt.dotplot <- dotplot(hgt, x = "Count", showCategory = nrow(data.frame(hgt)), label_format = 50)
hgt.dotplot

ggsave(hgt.dotplot +
       eheme(axis.text.y = element_text(size = 8)),
       file ="../temp/HGTdotplot_allRxns_RemissionWoHBMayoCorrection.pdf", width = 8, height = 5)
...
```{r subsetUpset, eval = FALSE, include = FALSE, echo = FALSE}

gsea.rxnExpr <- readRDS("Statistics/results/rxnExpr_GSEASubsystems_RemissionWoHBMayoCorrection.RDS")
gsea.PA <- readRDS("Statistics/results/PA_GSEASubsystems_RemissionWoHBMayoCorrection.RDS")
gsea.FVA <- readRDS("Statistics/results/FVA_GSEASubsystems_RemissionWoHBMayoCorrection.RDS")
...
There is(are) exactly
<!--r  sum(rowSums(vennMat.subsys) == ncol(vennMat.subsys))-->
subsystem(s) which is(are) found across all analysis. That is:

```{r subsysTable, eval = FALSE, include = FALSE, echo = FALSE}
...
topcor.ttests <- topcor[!is.na(Remission2_score), .(p.value = tryCatch({
                                                    t.test(Remission2_score)[["p.value"]]
                                                }, error = function(e){1.0}),
                                                Remission2_score_mean = mean(Remission2_score),
                                                Remission2_score_sum = sum(Remission2_score),
...
topcor.ttests[,Metabolite := factor(description, levels = unique(description[order(Remission2_score_sum)]))]
...
top100 <- topcor.ttests[padj < 0.05, rn]#[1:20]

plt.dat <- topcor[rn %in% top100,]
plt.dat <- merge(plt.dat, rxnAnno, by.x = "rn", by.y = "abbreviation", all.x = TRUE)
plt.dat[,Metabolite := factor(topcor.ttests[rn,Metabolite], levels = unique(topcor.ttests[order(Remission2_score_mean),Metabolite]))]
plt.dat[, compartment := "Colon"]
plt.dat[grep(".*\\[e\\]",rn), compartment := "Blood"]

plt.excor <- ggplot(plt.dat, aes(x = Metabolite, y = Remission2_score, color = main.direction)) +
    geom_boxplot() +
    theme_bw() +
...
topcor.sums.plt <- ggplot(topcor.ttests[order(padj)[1:25],], 
                          aes(x = Remission2_score_sum,
                              y = Metabolite,
                              size = NoRxns,
    ...
    theme_bw() 
topcor.sums.plt
ggsave(topcor.sums.plt, file = "./../temp/Top25Metabolites_RemissionWoHBMayoCorrection.pdf", width = 8, height = 4)
...
topcor.subsystems <- topcor.merge[rn %in% topcor.ttests[, rn] & !is.na(Remission2_score),
                            .(Remission2_score_mean = mean(Remission2_score,na.rm =TRUE),
                              Remission2_score_sum = sum(Remission2_score, na.rm = TRUE),
                              N_rxns = .N),
...
ggplot(topcor.subsystems, aes(x=Metabolite, y = subsystem, size = N_rxns,color = Remission2_score_sum)) +
