...
snakemake <- new("snakemake", input = list(
                        ...
                        lqmmModels = "../results/FVA/LQMMmodelsRangeFVA_MatjesAbsorption.colormore22-zeroBiomass.csv",
                        lmmModels = "../results/FVA/LMMmodelsRangeFVA_MatjesAbsorption.colormore22- _zeroBiomass.csv",
                        rxn_stats = "../results/FVA/PERMANOVAmodels_MatjesAbsorption.colormore22-distFVA_zeroBiomassvs~HB_Mayo_impu.csv",
...
colnames(rxnCount) <- gsub(".","_", colnames(rxnCount), fixed = TRUE)
...
vennMatAll <- vennMat
vennMat <- vennMatAll[,-2]
# plotting the venn diagram
#pdf("~/Work/miTarget/Reports/211206/VennDiagramHostRxns.pdf")
...
vennSig[vennSig$Subsystems == "","Subsystems"] <-  "Miscellaneous"
...
pp_R2 <-ggplot(data = vennSig, aes(y = Subsystems, x = Dist_R2,color =Subsystems)) +
    geom_jitter(height=0.25, width = 0,size=2)+
    xlab("Distance explained")+
    theme_bw() +
    theme(legend.position = "none",
    axis.text.y = element_blank(),
    axis.title.y = element_blank())

pp <- cowplot::plot_grid(pp_log2FC,
                         #pp_coeff,
                         pp_coeff_LMM,
                         pp_R2,
                         ncol =3,
                         rel_widths = c(2.6,1,1)
                        # ncol=4,
                        # rel_widths = c(2,1,1,1)
    )
pp
ggsave(pp, file = "~/Work/miTarget/Reports/211206/Statistics_consensusFinding.pdf")
