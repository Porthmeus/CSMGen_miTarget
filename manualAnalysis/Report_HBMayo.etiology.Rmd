...
output: 
    rmdformats::downcute:
        highlight: tango
        use_bookdown: TRUE
bibliography: ["/home/jan/Documents/References/Bibtex/My_Library.bib"]
link-citations: true
---
...
There are `r sum(rowSums(vennMat) == ncol(vennMat))` reactions which are associated in all analysis with HB/Mayo changes. Here is the complete list: `r DT::datatable(rxnAnno[abbreviation %in% rownames(vennMat[rowSums(vennMat) == ncol(vennMat),]),])`
...
The following plots is merely the sum/mean of all scores obtained for one metabolite across all significant reactions in the given condition.
$s_M = \sum_{i}^M\sqrt{r_{i}^2}*\sqrt{\beta_i^2}$  
...
The later plots are grouped by the subsystem - here is the equation for this:
$s_{SM} = \sum_{j}^S\sum_{i}^M\sqrt{r_{ij}^2}*\sqrt{\beta_{ij}^2}$ 
