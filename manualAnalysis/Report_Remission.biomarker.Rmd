...
coefs.rxnExpr <- fread("./Statistics/results/rxnExpr.GLMM.Remission.biomarker_coefs.csv")
sigs.rxnExpr <- coefs.rxnExpr[padj < 0.05,]
coefs.PA <- fread("./Statistics/results/PA.GLMM.Remission.biomarker_coefs.csv")
sigs.PA <- coefs.PA[padj < 0.05,]
coefs.FVA <- fread("./Statistics/results/FVA.GLMM.Remission.biomarker_coefs.csv")
...
gsea.rxnExpr <- readRDS("Statistics/results/rxnExpr.GSEA.Remission.biomarker_GSEAResult.RDS")
gsea.PA <- readRDS("Statistics/results/PA.GSEA.Remission.biomarker_GSEAResult.RDS")
gsea.FVA <- readRDS("Statistics/results/FVA.GSEA.Remission.biomarker_GSEAResult.RDS")
...
pca.plot <- ggplot(pca.x[Time_seq != max(Time_seq),], aes(x=PC1,y=PC2)) +
...
#
pca.common.plot <- ggplot(pca.common.x[Time_seq != max(Time_seq),], aes(x=PC1,y=PC2)) +
#    scale_color_gradient(low = "blue", high ="red") +
    geom_point(aes(color = Remission, shape = Treatment),size = 3) +
    theme_bw() +
    facet_grid(~Diagnosis) +
...
topcor <- all.Excors[est_src == "Remission" & aim == "biomarker"]
