...
coefs.rxnExpr <- fread("./Statistics/results/rxnExpr.GLMM.Remission.intervention_coefs.csv")
sigs.rxnExpr <- coefs.rxnExpr[padj < 0.05,]
coefs.PA <- fread("./Statistics/results/PA.GLMM.Remission.intervention_coefs.csv")
sigs.PA <- coefs.PA[padj < 0.05,]
coefs.FVA <- fread("./Statistics/results/FVA.GLMM.Remission.intervention_coefs.csv")
...
There are `r sum(rowSums(vennMat) == ncol(vennMat))` reactions which are associated in all analysis with Remission changes. Here is the complete list: `r DT::datatable(rxnAnno[abbreviation %in% rownames(vennMat[rowSums(vennMat) == ncol(vennMat),]),])`
...
sig.subs<- unique(c(data.frame(hgt)[,"ID"], data.frame(gsea)[,"ID"]))
...
gsea.rxnExpr <- readRDS("Statistics/results/rxnExpr.GSEA.Remission.intervention_GSEAResult.RDS")
gsea.PA <- readRDS("Statistics/results/PA.GSEA.Remission.intervention_GSEAResult.RDS")
gsea.FVA <- readRDS("Statistics/results/FVA.GSEA.Remission.intervention_GSEAResult.RDS")
...
An interesting finding was, that generally there were more reactions which, if present/active, there was a reduction in disease activity. This can be either an effect that certain reactions are really blocked/reduced in activity if Remission scores are high or it is a result of a reduced metabolic diversity in higher inflammation states. To test that one can correlate the Remission score with the diversity measures of the different analysis.

```{r diversity}

diversity <- fread("Statistics/results/diversityMeasures.csv")
pairs.panels(diversity[,.(Remission,Remission,HB_Mayo_impu,richness.PA, shannon.rxnExpr, shannon.FVA)],
           method = "spearman",
           stars = TRUE,
           main = "Spearman's rho")
...
mod.richness.PA <- glmer(data = diversity,
                        factor(Remission) ~ richness.PA + (1|PatientID),
                        family = binomial)
mod.shannon.rxnExpr <- glmer(data = diversity,
                        factor(Remission) ~ shannon.rxnExpr + (1|PatientID),
                        family = binomial)
mod.shannon.FVA <- glmer(data = diversity,
                        factor(Remission) ~ shannon.FVA + (1|PatientID),
                        family = binomial)
...
There is a decrease of diversity in FVA ranges with the increase in Remission signal - that is interesting indeed.



## PCA for clustering

Another idea is to use the different significant reactions and use the data to make a ordination to get a feeling how well we can separate the different samples form each other by Remission scores.
...
pca.plot <- ggplot(pca.x, aes(x=PC1,y=PC2)) +
    ...
    geom_point(aes(color = Remission, shape = Treatment),size = 3) +
    theme_bw() +
    facet_grid(~Diagnosis) +
    labs(y = paste0("PC2 (", round(pca.var[2],2),"%)"),
         x = paste0("PC1 (", round(pca.var[1],2),"%)"),
         color = "Remission score")
pca.plot
...
##    scale_color_gradient(low = "blue", high ="red") +
#    geom_point(aes(color = Remission, shape = Diagnosis),size = 3) +
#    theme_bw() +
#    facet_grid(~Diagnosis) +
...
topcor <- all.Excors[est_src == "Remission" & aim == "intervention",]



# filter the exchange reactions, which show a significant effect which is different from 0
topcor.ttests <- topcor[est_src == "Remission", .(p.value = tryCatch({
...
plt.excor <- ggplot(topcor[est_src == "Remission" & rn %in% top100 ,], aes(x = Metabolite, y = sin(score_abs_mean)*log(abs(1/score_abs_mean)+1), fill = main.direction)) +
    geom_boxplot() +
    theme_bw() +
...
topcor.sums.plt <- ggplot(topcor.ttests[order(abs(score_absabs_sum),decreasing = T)[1:25],], 
    ...
    labs(x = "Summed importance score", title = "Remission associated metabolites") +
    theme_bw() 
topcor.sums.plt
...
topcor.sums.plt <- ggplot(topcor.ttests[order(abs(score_absabs_sum),decreasing = T)[1:25],], 
    ...
    geom_point() +
    geom_vline(linetype = 2, color = "red", xintercept = 0) +
    labs(x = "Mean importance score", title = "Remission associated metabolites") +
    theme_bw() 
topcor.sums.plt
...
#topcor.subsystems <- topcor.merge[rn %in% topcor.ttests[NoRxns == 1, rn] & !is.na(Remission_score),
...
#topcor.subsystems[, Remission_score_direction := (as.integer(Remission_score_sum>0)*2)-1]
#topcor.subsystems[, compartment := "Colon"]
#topcor.subsystems[grep(".*\\[e\\]",rn), compartment := "Blood"]
#
#ggplot(topcor.subsystems[Remission_score_direction <0,], aes(x=Metabolite, y = subsystem)) +
#    scale_shape_manual(values=c(25,24))+
#    geom_point(aes(shape = as.factor(Remission_score_direction), fill = main.direction, color = main.direction)) +
