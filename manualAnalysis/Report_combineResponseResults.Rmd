...
There are `r sum(rowSums(vennMat) == ncol(vennMat))` reactions which are associated in all analysis with treatment response changes. Here is the complete list: `r DT::datatable(rxnAnno[abbreviation %in% rownames(vennMat[rowSums(vennMat) == ncol(vennMat),]),])`
