...
rule extractModels:
    input:
        TPM = "results/data/SPLIT_TPM_emed_future/SPLIT_{sample}.csv",
    ...
    output:
        "results/data/extractedModels/{diet}.{model}-{sample}.csv"
    log: "logs/extractModels_{diet}.{model}-{sample}.log"
    params:
        thrsld = 75, # percentile of data
